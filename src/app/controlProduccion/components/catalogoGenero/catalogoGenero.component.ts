...
@Component({
    ...
    templateUrl: './catalogoGenero.component.html'
})
export class CatalogoGeneroComponent extends BaseComponent implements OnInit {
    ...
    catalogoController = "catalogoGenero";
        ...
        if (this.elementoSeleccionado != null && this.elementoSeleccionado.id != null && !this.esNuevo) {
            this.confirmationService.confirm({
                ...
                accept: () =>
                {
                    ...
                    this.catalogoGenericoService.eliminarCatalogo(this.catalogoController, JSON.stringify({ id: this.elementoSeleccionado.id }))
                    .then(resultado => {
                        this.blockService.cleanBlock();
                        this.elementoEliminado()
                    }, estatus => { this.mensajeService.mandarMensajeError("Ocurrió un error al eliminar el elemento [" + this.elementoSeleccionado.id + "] " + this.elementoSeleccionado.nombre); this.blockService.cleanBlock(); }); 
                }
            } );
        } else {
    ...
    ejecutarBotonGuardar() {
        if (this.elementoGuarda != null) {
            ...
            if (this.esNuevo) {
                // Alta registro
                this.catalogoGenericoService.registrarCatalogo(this.catalogoController, this.elementoGuarda)
                .then(elemento => { this.obtenerDatos(); this.mostrarMensaje("Registró", elemento); }, resultado => { this.mensajeService.mandarMensajeError("Ocurrió un error al guardar el elemento [" + this.elementoGuarda.id + "] " + this.elementoGuarda.nombre); this.blockService.cleanBlock();
                });
            }
            else {
                // Edicion registro
