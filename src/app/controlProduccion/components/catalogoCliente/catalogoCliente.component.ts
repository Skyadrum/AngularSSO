...
export class CatalogoClienteComponent extends BaseComponent implements OnInit {
    ...
    catalogoController="catalogoClientes";
            ...
            this.confirmationService.confirm({
                ...
                accept: () => {
                    ...
                    this.catalogoGenericoService.eliminarCatalogo(this.catalogoController, JSON.stringify({ id: this.elementoSeleccionado.id })).then(resultado => {this.blockService.cleanBlock(); this.elementoEliminado()}, estatus => {
                        this.mensajeService.mandarMensajeError("Ocurrió un error al eliminar el elemento [" + this.elementoSeleccionado.id + "] " + this.elementoSeleccionado.nombre);
                    });
                }
            });
        } else {
        ...
        if (this.elementoGuarda != null) {
            ...
            if (this.esNuevo) {
                ...
                this.catalogoGenericoService.registrarCatalogo(this.catalogoController, this.elementoGuarda).then(elemento => { this.obtenerDatos(); this.mostrarMensaje("Registró", elemento) }, resultado => {
                    this.mensajeService.mandarMensajeError("Ocurrió un error al guardar el elemento [" + this.elementoGuarda.id + "] " + this.elementoGuarda.nombre); this.blockService.cleanBlock();
                });
            }
            else {
                // Edicion registro
    ...
    obtenerCatalogos() {
        this.catalogoGenericoService.obtenerCatalogos(this.catalogoController).then(elementos => this.elementos = elementos, resultado => {
            this.mensajeService.mandarMensajeError("Ocurrió un error al obtener el Catálogo Cliente");
        });
    }
